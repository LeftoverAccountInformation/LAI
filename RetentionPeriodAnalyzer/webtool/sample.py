...
sys.stdout = stdout
###############################################################
sys.path.insert(0, "/Users/preethis/Documents/pystatparser")
...
def _exceptioncheck(sentence, verb, noun):
    ...
    if len(s) >= 2 and len(s) <= 5:
        for i in range(len(s)-1):
            if _lemmacheck(s[i], verb, 'v') and _lemmacheck(s[i+1], noun, 'n'):
                return True
            if i <= len(s)-3 and _lemmacheck(s[i], verb, 'v') and _lemmacheck(s[i+2], noun, 'n'):
                return True

    if len(s) >= 3 and len(s) <= 30:
        pv = ps = pn = -1
        for i in range(len(s)):
    ...
    if len(s) >= 3 and len(s) <= 16:
        pn = pb = pv = -1
        for i in range(len(s)):
            pn = i if _lemmacheck(s[i], verb, 'n') or _lemmacheck (s[i], 'data', 'n') else pn
    ...
    if len(s) <= 20:
        for i in range(len(s)):
            if _lemmacheck(s[i], verb, 'n') and _lemmacheck(s[i], noun, 'v') and s[i+5] == "Endomondo":
                return True
...
def verbcheck(verb, noun, cst):
    ...
    while True:
        try:
            st = gen.next()
            if st.height() == 2 and st.label() in vlabels and _lemmacheck(verb, st[0], 'v'): #add just st.label() in vlabels and test it
...
def prepcheck(noun, cst):
    gen = cst.subtrees()
    for leaf in gen.next().leaves(): #checks the final (leaf) nodes with the noun and if it matches noun is found
