# This script is a clone of delete.py from lai tool
# The differences:
# 1. No dot file (we implement ADF Analysis on the fly, i.e., lac_adf.sh)
# 2. No need APK file as parameter, only PACKAGE_NAME (use init3 instead of init2)
# 3. EMAIL and PASSWORD can be supplied as parameters instead of using default values
# Usage: 
#    python cleanup.py <package-name> <app-email> <app-password>

from commons import _UNINSTALL_FLAG, EMAIL, PASSWORD, USERNAME, BIRTHDAY, FIRSTNAME, LASTNAME, PHONENUMBER, HEIGHT, WEIGHT, AGE, GENDER, LOCATION, ZIPCODE, SSN, DLN, CREDITCARD, init3, find_edits, dump_text
...
if len(sys.argv) == 4:
    ...
    print("Usage: python cleanup.py <package-name> [<app-email> <app-password>]")
    sys.exit(1)
